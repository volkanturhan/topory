# Builds a complete topory release — the portable self-contained exe and the
# Windows installer — both named with the version read from topory.csproj.
#
#   .\tools\release.ps1                   -> artifacts land in dist\release
#   .\tools\release.ps1 -OutDir <folder>  -> also copied into <folder>
#
# The -OutDir hook lets the outer workspace collect the build into builds\topory
...
Write-Output "Building topory v$version"
...
$portable = Join-Path $releaseDir "topory-v$version.exe"
$setup    = Join-Path $releaseDir "topory-setup-v$version.exe"
Copy-Item (Join-Path $selfContainedDir 'topory.exe') $portable -Force
Copy-Item (Join-Path $root "dist\installer\topory-setup-v$version.exe") $setup -Force
